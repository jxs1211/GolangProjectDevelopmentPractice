...
Here's a shell script that automates the Kubernetes container debugging process with Delve, including customizable parameters:

```bash
#!/bin/bash

# Kubernetes Delve Debugger Setup Script
# Usage: ./k8s-debug.sh [OPTIONS]
...
usage() {
  ...
  echo "Options:"
  echo "  -p, --pod-name       Pod name (required)"
  echo "  -c, --container      Container name (default: $CONTAINER)"
  echo "  -n, --namespace      Namespace (default: $NAMESPACE)"
  echo "  -k, --kubeconfig     Kubeconfig path (default: $KUBECONFIG)"
  echo "  -d, --debug-port     Debugger port (default: $DEBUG_PORT)"
  echo "  -a, --app-port       Application port (default: $APP_PORT)"
  echo "  -i, --pid            Target process ID (default: $TARGET_PID)"
  echo "  -l, --dlv-path       Local Delve path (default: $DLV_PATH)"
...
if [[ -z "$POD_NAME" ]]; then
  echo "Error: Pod name must be specified with -p or --pod-name"
  usage
fi
...
# Check if dlv is available locally
if [[ ! -f "$DLV_PATH" ]]; then
  echo "Error: Delve debugger not found at $DLV_PATH"
  exit 1
fi

echo "Starting Kubernetes debugging setup for pod $POD_NAME"

# Step 1: Show processes in container
echo -e "\n[1/6] Listing processes in container $CONTAINER..."
kubectl exec -it "$POD_NAME" -c "$CONTAINER" -n "$NAMESPACE" \
  --kubeconfig "$KUBECONFIG" -- ps -ef

# Step 2: Copy Delve to container
echo -e "\n[2/6] Copying Delve debugger to container..."
kubectl cp "$DLV_PATH" "$POD_NAME:/usr/local/bin/dlv" -c "$CONTAINER" \
  -n "$NAMESPACE" --kubeconfig "$KUBECONFIG"

# Step 3: Attach Delve to target process
echo -e "\n[3/6] Attaching Delve to PID $TARGET_PID..."
echo "Debugger will be available on port $DEBUG_PORT"
kubectl exec -it "$POD_NAME" -c "$CONTAINER" -n "$NAMESPACE" \
  --kubeconfig "$KUBECONFIG" -- \
  dlv attach "$TARGET_PID" --headless --listen=:"$DEBUG_PORT" \
  --api-version=2 --accept-multiclient --continue > /dev/null 2>&1 &

# Wait for debugger to start
sleep 3

# Step 4: Set up port forwarding for debugger
echo -e "\n[4/6] Setting up port forwarding for debugger..."
kubectl port-forward "$POD_NAME" "$DEBUG_PORT:$DEBUG_PORT" -n "$NAMESPACE" \
  --kubeconfig "$KUBECONFIG" > /dev/null 2>&1 &

# Step 5: Set up port forwarding for application (optional)
echo -e "\n[5/6] Setting up port forwarding for application on port $APP_PORT..."
kubectl port-forward "$POD_NAME" "$APP_PORT:$APP_PORT" -n "$NAMESPACE" \
  --kubeconfig "$KUBECONFIG" > /dev/null 2>&1 &

# Step 6: Provide connection instructions
echo -e "\n[6/6] Debugging setup complete!"
echo -e "\nTo connect to the debugger, run:"
echo "dlv connect localhost:$DEBUG_PORT"
echo -e "\nTo access the application:"
echo "curl http://localhost:$APP_PORT"

echo -e "\nPress Ctrl+C to stop debugging and clean up"

# Cleanup on exit
cleanup() {
  echo -e "\nCleaning up..."
  pkill -f "kubectl port-forward $POD_NAME $DEBUG_PORT:$DEBUG_PORT"
  pkill -f "kubectl port-forward $POD_NAME $APP_PORT:$APP_PORT"
  exit 0
}
...
1. Save the script as `k8s-debug.sh`
...
3. Run with required parameters:
   ```bash
   ./k8s-debug.sh -p ccp-apiserver-S9ddcd4ffb-k6ndr
   ```
4. For full customization:
   ```bash
   ./k8s-debug.sh -p my-pod -c my-container -n my-ns -k ~/.kube/config -d 5000 -a 8080 -i 123 -l ~/bin/dlv
   ```
...
2. **Automated Process**:
   ...
   - Copies Delve to container
   - Attaches debugger to specified PID
   - Sets up port forwarding for both debugger and application
   - Provides connection instructions

3. **Cleanup**:
   - Automatically stops port forwarding when script is terminated
   - Handles Ctrl+C gracefully

4. **Error Handling**:

   - Validates required parameters
   - Checks for kubectl and Delve availability

This script provides a complete, automated solution for debugging Kubernetes containers with Delve while maintaining flexibility through customizable parameters.
```
```
# Check which port is running inside container
kubectl get pod -o wide
# Login to the node where pod is running
docker inspect <containerid> | grep Pid
nsenter -n -t <Pid>

# Build and push apiserver image
make images TARGETS=apiserver OUTPUT_TYPE=registry
# Output:
# cb40a52b6db6: Layer already exists
# 861dddb4c520: Layer already exists
# 0a25f4a8-amd64: digest: sha256:eef92c65417f60b142544ee2a7497090ff1c69e9e9dd5546075941e70db7f4d9 size: 2823

kubectl get deployment -n ccp ccp-apiserver -o yaml | grep imagePullPolicy
# imagePullPolicy: Always

# Get metrics-server logs
kubectl logs -n kube-system -f metrics-server-65d5fbf8d4-m7mcg -c metrics-server > metrics-server.log

# Set default namespace to keda
kubectl config set-context --current --namespace=keda

# Update keda-external-scaler image
kubectl set image deployment/keda-external-scaler-deployment keda-external-scaler=cd.artifactory.cibt/lo02x0-1st-docker-dev-cd/keda-rocketmq-scaler:7ebeaa0-amd64 -n keda-external-scaler-ns

# Get all ScaledObjects across namespaces using backup kubeconfig
kubectl get so --all-namespaces --kubeconfig=/root/.kube/config-bk

# Get events for specific ScaledObject using backup kubeconfig
kubectl get events --field-selector involvedObject.kind=ScaledObject,involvedObject.name=ingress-test-so --kubeconfig=/root/.kube/config-bk

# Force delete terminating namespace
kubectl get namespace keda -o json | jq 'del(.spec.finalizers)' | kubectl replace --raw "/api/v1/namespaces/keda/finalize" -f -

# Start kubectl proxy
kubectl proxy

# Force finalize namespace via API using curl
curl -H "Content-Type: application/json" -X PUT -d '{"apiVersion":"v1","kind":"Namespace","metadata":{"name":"keda"},"spec":{"finalizers":[]}}' http://localhost:8001/api/v1/namespaces/keda/finalize

# Run KEDA locally with debug logging
cd /shenxianjie/keda-core/keda
make run ARGS="--zap-log-level=debug"

# Search for cluster version references
grep -rnw . -e 'v1/cluster/version'

# Search for sarama usage in Go files
grep -rnw /root/shenxianjie/keda-core/keda -e 'sarama' --include='*.go'

# Check go.mod for specific version
grep -rnw go.mod -e 'v0.31.7'

# Find kafka_scaler.go
find . -iname "kafka_scaler.go"

# Replace import path in Go files
find /path/to/your/project -name '*.go' | xargs sed -i 's|g.hz.netease.com/base-cloud/rocketmq-client-go/v2|cd.gitlab.cibt/cluster-component/keda-rocketmq-scaler/pkg/client/rocketmq/|g'

# Search for inplaceSet API references
grep -rniw . -e 'cd.gitlab.cibt/l501x0/cibdeployment/pkg/api/inplaceset'

# Extract first field using awk
awk -F '.' '{print $1}'

# Replace import path for inplaceSet Go types
sed -i -e 's|cd.gitlab.cibt/501x0/cibdeployment/pkg/api/inplaceset/v1alpha1|cd.gitlab.cibt/501x0/cibdeployment/pkg/client/clientset/versioned/typed/apps/v1alpha1|g'

# Generate TLS certs
mkdir -p /certs
openssl req -newkey rsa:2048 -nodes -subj '/CN=localhost' -addext 'subjectAltName=DNS:localhost' -nodes -keyout /certs/tls.key -x509 -days 3650 -out /certs/tls.crt
cp /certs/tls.crt /certs/ca.crt

# Set environment variables for KEDA
export KEDACUSTEROBJECTNAMESPACE="keda"
export WATCHNAMESPACE=""

# Run KEDA with debug logging
make run ARGS="--zap-log-level=debug"

# Debug container in pod with dlv

## Find the debug process ID of container
kubectl exec -it ccp-apiserver-59ddcd4ffb-k6ndr -c appserver -n ccp --kubeconfig=/root/.kube/config-bk -- bash
# Inside container:
ps -ef
# Output:
# UID   PID  PPID  C STIME TTY    TIME CMD
# root    1     0  0 19:00 ?     00:00:02 /bin/apiserver --enable-compare
# root  207     0  0 19:21 pts/1 00:00:00 bash
# root  249   207  0 19:21 pts/1 00:00:00 ps -ef

## Copy dlv binary to container
kubectl cp /usr/local/bin/dlv ccp-apiserver-59ddcd4ffb-k6ndr:/usr/local/bin/dlv -c appserver -n ccp --kubeconfig=/root/.kube/config-bk

## Debug running process with dlv attach <pid>
kubectl exec -it ccp-apiserver-59ddcd4ffb-k6ndr -c appserver -n ccp --kubeconfig=/root/.kube/config-bk -- dlv attach 1 --headless --listen=:4000 --api-version=2 --accept-multiclient --continue
# Output:
# API server listening at: []:4000
# 2025-07-10T19:49:41+08:00 warning layer=rpc Listening for remote connections (connections are not authenticated nor encrypted)

## Forward dlv attach's port for connection
kubectl port-forward pod/ccp-apiserver-59ddcd4ffb-k6ndr 4000:4000 -n ccp --kubeconfig=/root/.kube/config-bk
# Output:
# Forwarding from 127.0.0.1:4000 -> 4000
# Forwarding from [::1]:4000 -> 4000
# Handling connection for 4000

## Connect to running process through dlv attach's port
dlv connect localhost:4000
# Breakpoint hit in:
# k8s.io/client-go/dynamic.(*dynamicResourceClient).List
# /root/gwx/ccp/vendor/k8s.io/client-go/dynamic/simple.go:300 (PC: 0x1812ad6)
...
# 		return nil, err
# 	}
# 	result := c.client.client.Get().AbsPath(c.makeURISegments("")...).SpecificallyVersionedParams(&opts, dynamicParameterCodec, versionv1).Do(ctx)
# 	if err := result.Error(); err != nil {
# 		return nil, err
# 	}

## Forward app's HTTP port
kubectl port-forward pod/ccp-apiserver-59ddcd4ffb-k6ndr 8989:8989 -n ccp --kubeconfig=/root/.kube/config-bk

## Curl the API
curl http://20.8.247.86:33770/v1/namespaces/test789/otherresources?gvr=group:version:v1.resource:bindings&labelSelector=
```
